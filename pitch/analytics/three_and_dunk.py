...
Windows: three = release - 3 s to release + 2 s (the swish is inside), dunk = dunk - 3 s
to dunk + 2 s. The portrait clip is scaled to 1080 px height and centred on a
...
def main() -> int:
    ...
    ap.add_argument("--three", type=float, required=True, help="time (s) the ball leaves the hand on the three")
    ap.add_argument("--dunk", type=float, required=True, help="time (s) of the dunk")
    ...
    part(args.src, max(0.0, args.three - args.before), dur, "three pointer, tracked live", a)
    part(args.src, max(0.0, args.dunk - args.before), dur, "dunk", b)
    lst = tmp / "list.txt"
